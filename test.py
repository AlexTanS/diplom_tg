...
API_TOKEN = '8188076440:AAE1v-T8PAUJZVfCTa9nQR-0bvHnZiYHXzA'
...
async def end_services(message: types.Message, state: FSMContext):
    ...
    if d is not None:  # получаю словарь с ответом {'test': 'test', 'test2': 'test2'}
        await message.reply("Спасибо за ввод данных!", reply_markup=ReplyKeyboardRemove())
    else:
        await message.answer("Произошла ошибка при отправке запроса.")

    # в консоль
    # print(f"Билет: {data['id_ticket']}")
    # print(f"Пароль: {data['password']}")
    # print(f"Выбор: {data['service']}")
    print(f"Пришло: {d}")
